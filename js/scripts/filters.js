...
var tmFilters = {
  ...
  config: {
    filters_cont:
        '#filters',  //Селектор оберточного элемента блоков с фильтрами
    ...
    filter_slider: 'div.range-slider',  //Селектор слайдеров (ползунок для
                                        //числовых значений)
    filter_slider_cont: 'div.filter__slider',  //Селектор контейнера со слайдером
    pages_cont1: '#pages',  //Селектор контейнера с постраничной навигацией
    pages_cont2: '#pages2',  //Селектор второго контейнера с постраничной
                             //навигацией. Если нет, оставить пустым.
    active_page_selector: '.current',  //селектор номера текущей страницы внутри
                                       //контейнера (pages_cont)
    filters_type: 'default',  //Тип фильтрации. Возможные значения:
                              // default (показ числа товаров по каждому фильтру
                              // и блокирование пустых вариантов), only_block
                              // (только блокирование пустых париантов), none
                              // (не показывать цифры и не блокировать)
    filter_delay: 700,  //Задержка до отправления запроса на сервер
                        //(сбрасывается после каждой отметки фильтра)
  ...
  init: function(conf) {
    ...
    $('input,select', this.config.filters_cont).on('change', function(event) {
  ...
  slidersInit: function() {
    ...
    if ($(tmFilters.config.filter_slider, tmFilters.config.filters_cont)
            .length > 0) {
      $(tmFilters.config.filter_slider, tmFilters.config.filters_cont)
          .each(function(i) {

            var this_slider = $(this), slider_wrapper = this_slider.parent(),
                flt_name = tmFilters.getFilterName(
                    $('input:text:eq(0)', slider_wrapper).attr('name')),
            ...
            if (tmFilters.config.multi_currency &&
                flt_name == tmFilters.config['price_field']) {
            ...
            this_slider.slider({
              ...
              start: function(event, ui) {
                //Сохраняем значение чтобы потом иметь возможность вернуть
                //предыдущее
            ...
            $('input:text', slider_wrapper).on('keyup', function(e) {
              ...
              tmFilters.timer2 = setTimeout(function() {
                var curMinValue =
                        parseFloat(tmFilters.getNumber(minField.val())),
                    curMaxValue =
                        parseFloat(tmFilters.getNumber(maxField.val()));
                ...
                if (curMinValue > curMaxValue) {
                  if (curMinValue < maxValue)
                    curMaxValue = curMinValue;
                  else if (curMaxValue > minValue)
                    curMinValue = curMaxValue;
                }
  ...
  filtersPreSubmit: function(event) {
    ...
    if (!is_select && tmFilters.config['filters_type'] != 'none' &&
        result_ids.length == 0) {
      //если товаров не найдено, изменяем значение на ближайшее
      if (typeof event != 'undefined' &&
          $(event.target).is(tmFilters.config.filter_slider)) {
  ...
  pushState: function() {
    ...
    $('input[name="page_id"]', tmFilters.config.filters_cont)
        .prop('disabled', false);
    ...
    if (!tmFilters.filtered && !tmFilters.sorted) {
      ...
      for (var i in form_data) {
        if (!form_data.hasOwnProperty(i)) continue;
        if ($.inArray(
                form_data[i].name, ['page', 'sortby', 'sortdir', 'limit']) >
            -1) {
          if (form_data[i].name != 'page' || form_data[i].value != 1) {
            search_uri += '&' + form_data[i].name + '=' + form_data[i].value;
            form_data_push.push(form_data[i]);
    ...
    window.History.pushState(
        form_data_push, $('title').text(), loc_path + search_uri);
  ...
  getFilters: function() {
    ...
    $(tmFilters.config.filter_slider).each(function(i) {
      ...
      var slider_limits =
          [$(this).slider('option', 'min'), $(this).slider('option', 'max')];

      if (flt_values[0] !== slider_limits[0] ||
          flt_values[1] !== slider_limits[1]) {
  ...
  currRate: function(flt_name, value, type) {
    ...
    if (flt_name == this.config['price_field'] &&
        typeof tmFiltersOptions != 'undefined' &&
        tmFiltersOptions.currency_rate) {
      var curr_default = tmFiltersOptions.currency_default ?
          parseInt(tmFiltersOptions.currency_default) :
          1;
      var curr_rates_arr = tmFiltersOptions.currency_rate || [];

      var shk_cindex = document.cookie.indexOf('shk_currency=') > -1 ?
          document.cookie.indexOf('shk_currency=') +
              new String('shk_currency=').length :
          -1;
  ...
  filter: function(flt_name, flt_value) {
    ...
    for (var i in window.flt_data.products) {
      if ($.inArray(
              parseInt(window.flt_data.products[i].id), tmFilters.result_ids) ==
          -1) {
        continue;
      }
      ...
      if (!!window.flt_data.products[i][flt[0]]) {
        switch (flt[1]) {
          case 'between':
            ...
            var p_value = parseFloat(
                window.flt_data.products[i][flt[0]].replace(',', '.'));
          ...
          case 'like':
            ...
            for (var ii in flt_value) {
              if (window.flt_data.products[i][flt[0]].indexOf(
                      tmFilters.config.guard_key + flt_value[ii] +
                      tmFilters.config.guard_key) > -1) {
                remove = false;
                break;
              }
            }
          ...
          case 'eq':
            ...
            if ($.inArray(window.flt_data.products[i][flt[0]], flt_value) >
                -1) {
              remove = false;
            }
  ...
  updateMarkerCount: function(elem, count) {
    ...
    var elem_parent = elem.closest('.filter_row');
  ...
  setbackSlider: function(event) {
    ...
    var slider_limits = [
      this_slider.slider('option', 'min'), this_slider.slider('option', 'max')
    ];
    ...
    for (var i in f_state) {
      ...
      // slider
        ...
        if (f_state[i].from && f_state[i].to) {
          ...
          $('#' +
            'range_' + f_name)
              .slider('values', 0, f_state[i].from);
          $('#' + f_name + 'Max').val(f_state[i].to);
          $('#' +
            'range_' + f_name)
              .slider('values', 1, f_state[i].to);
        }
        ...
        if ($('input:checkbox', tmFilters.config.filters_cont).length > 0) {
          var input_name = $.inArray(f_name, tmFilters.config.multitags) > -1 ?
              ...
              'f_' + f_name + '[]';
          var f_input =
              $('input[name="' + input_name + '"]:checkbox',
                tmFilters.config.filters_cont);
          ...
          var f_select = $(
              'select[name="f_' + f_name + '"]', tmFilters.config.filters_cont);
  ...
  getUrlVars: function() {
    ...
    var location_hash = document.location.search.length >= 3 ?
        document.location.search.substr(1) :
        window.location.hash.substr(1);
    location_hash = decodeURIComponent(location_hash.replace(/\+/g, ' '));
    if (location_hash.indexOf('?') > -1)
      location_hash = location_hash.substr(location_hash.indexOf('?') + 1);
    ...
    jQuery.ajax({
      ...
      success: function(response) {
        ...
        if (typeof response.prod_list != 'undefined') {
          ...
          $('html,body').animate({
            scrollTop:
                Math.round($(tmFilters.config.products_cont).position().top)
          });
        }
        ...
        if (typeof tm_onFilterAfter == 'function') {
          tm_onFilterAfter(
              response.total, response.pageCount, response.onPageLimit);
        }
  ...
  ajaxPreload: function(target, action) {
    ...
    if (action == true) {
      ...
      target.css({'position': 'relative', 'opacity': 0.4})
          .prepend('<div id="ajax_loader"></div>');

      $('#ajax_loader').css({
        'width': target.width() + 'px',
        'height': target.height() + 'px',
        'background': 'url(\'' + tmFilters.config.base_url +
            tmFilters.config.ajax_loader +
  ...
  ajaxPagesInit: function() {
    ...
    $(document)
        ...
        .keyup(function(event) {
          ...
          if (tmFilters.keyCtrl && event.which == 37) {
            var next_link = $(tmFilters.config.active_page_selector,
                              tmFilters.config.pages_cont1)
                                .prev('a');
            if (next_link.length == 0)
              next_link = $(tmFilters.config.active_page_selector,
                            tmFilters.config.pages_cont1)
                              .parent()
                              .prev()
                              .children('a');
            var next_page =
                next_link.length > 0 ? parseInt(next_link.text()) : 0;
          }
    ...
    if (tmFilters.config.pages_cont2.length > 0)
      pages_selector += ',' + tmFilters.config.pages_cont2 + ' a';
  ...
  setFormValue: function(name, value) {
    ...
    if ($('input[name="' + name + '"],select[name="' + name + '"]',
          tmFilters.config.filters_cont)
            .length == 0) {
      $('form', tmFilters.config.filters_cont)
